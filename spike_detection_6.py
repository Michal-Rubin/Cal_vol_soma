...
def detect_bursts_from_vm(trace_idx, spike_heights_interpolated, complex_bursts_dict, all_spikes, fr,
    ...
    v6 CS classification:
    1) remove +/-3 frames around each spike on the raw trace
    2) linearly interpolate missing samples
    3) apply a 20 ms moving-average low-pass
    4) classify CS when burst has >=2 spikes and peak subthreshold z-score >=5
    5) CS start/end are z=1.5 rising/falling crossings around the depolarization bump
    6) non-CS burst start/end are first_spike-5 to last_spike+5 frames
    """
    ...
    # Build CS subthreshold trace from raw trace_idx:
    # remove +/-3 frames around spikes, interpolate, then 20 ms moving average.
    vm_sub = np.asarray(trace_idx, dtype=float).copy()
    vm_sub[~np.isfinite(vm_sub)] = np.nan
    spike_times = np.sort(np.unique(all_spikes[(all_spikes >= 0) & (all_spikes < n)]))
    for spk in spike_times:
        s = max(0, int(spk) - 3)
        e = min(n, int(spk) + 4)  # inclusive +/-3
        vm_sub[s:e] = np.nan
    vm_sub = interpolate_nan_segment(vm_sub)

    if not np.any(np.isfinite(vm_sub)):
        vm_sub = np.zeros(n, dtype=float)
    else:
        fill_val = float(np.nanmedian(vm_sub))
        vm_sub = np.where(np.isfinite(vm_sub), vm_sub, fill_val)

    ma_frames = max(1, int(np.round(0.02 * fr)))
    if ma_frames > 1:
        kernel = np.ones(ma_frames, dtype=float) / float(ma_frames)
        Vm = np.convolve(vm_sub, kernel, mode='same')
    else:
        Vm = vm_sub.copy()

    vm_mean = float(np.nanmean(Vm)) if np.any(np.isfinite(Vm)) else 0.0
    vm_std = float(np.nanstd(Vm)) if np.any(np.isfinite(Vm)) else 0.0
    if (not np.isfinite(vm_std)) or vm_std < 1e-12:
        Vm_z = np.zeros_like(Vm)
    else:
        Vm_z = (Vm - vm_mean) / vm_std
    ...
    # Find depolarization windows from positive z-scored subthreshold Vm
    Vm_finite = np.where(np.isfinite(Vm), Vm, 0.0)
    Vm_z_finite = np.where(np.isfinite(Vm_z), Vm_z, 0.0)
    ...
    while ri < len(rise_idx) and fi < len(fall_idx):
        ...
        if fi >= len(fall_idx):
            break
        f = fall_idx[fi]
        depol_windows.append((int(r), int(f) - 1))
        ri += 1
        fi += 1
    ...
    cs_min_spikes = max(2, int(min_num_spikes))
    cs_z_threshold = 5.0
    cs_crossing_z = 1.5
    noncs_pad_frames = 5

    def _cs_bounds_from_crossing(win_start, win_end, zthr=1.5):
        """
        First and last CS frames are defined by rising/falling crossings of zthr.
        Implemented as contiguous region >= zthr around the local depolarization peak.
        """
        s = max(0, int(win_start))
        e = min(n - 1, int(win_end))
        if e < s:
            return s, s

        segz = Vm_z_finite[s:e + 1]
        if segz.size == 0 or not np.any(np.isfinite(segz)):
            return s, e

        peak_rel = int(np.nanargmax(segz))
        if (not np.isfinite(segz[peak_rel])) or (segz[peak_rel] < zthr):
            return s, e

        left = peak_rel
        while left > 0 and segz[left - 1] >= zthr:
            left -= 1

        right = peak_rel
        while right < (len(segz) - 1) and segz[right + 1] >= zthr:
            right += 1
    ...
    for win_start, win_end in depol_windows:
        ...
        window_z = Vm_z_finite[win_start:win_end + 1]
        peak_z = np.nanmax(window_z) if len(window_z) > 0 else np.nan

        n_spikes = len(spikes_in_window)
        # CS duration: z=1.5 crossing bounds
        cs_start, cs_end = _cs_bounds_from_crossing(win_start, win_end, zthr=cs_crossing_z)
        cs_duration_ms = (cs_end - cs_start + 1) * 1000 / fr
        meets_criteria = (
            (n_spikes >= cs_min_spikes)
            and np.isfinite(peak_z)
            and (peak_z >= cs_z_threshold)
            and (cs_duration_ms >= float(cb_duration_threshold))
        )

        if meets_criteria:
            w = Vm_finite[cs_start:cs_end + 1]
            peak_raw = np.nanmax(w) if len(w) > 0 else np.nan
            baseline = _local_baseline(cs_start)
            peak_amp = peak_raw - baseline if baseline_subtract else peak_raw
            duration_ms = cs_duration_ms
            auc = np.trapezoid(np.clip(w, 0, None), dx=1 / fr) if len(w) > 0 else 0.0
            ...
            complex_bursts.append((cs_start, cs_end))
            complex_spikes.extend(spikes_in_window.tolist())
        elif n_spikes > 1:
            # non-CS burst duration: fixed +/-5 frames around first/last spike
            b_start = max(0, int(spikes_in_window[0]) - noncs_pad_frames)
            b_end = min(n - 1, int(spikes_in_window[-1]) + noncs_pad_frames)
            w = Vm_finite[b_start:b_end + 1]
            peak_raw = np.nanmax(w) if len(w) > 0 else np.nan
            baseline = _local_baseline(b_start)
            peak_amp = peak_raw - baseline if baseline_subtract else peak_raw
            duration_ms = (b_end - b_start + 1) * 1000 / fr
            auc = np.trapezoid(np.clip(w, 0, None), dx=1 / fr) if len(w) > 0 else 0.0
            ...
            simple_bursts.append((b_start, b_end))
            simple_burst_spikes.extend(spikes_in_window.tolist())
        else:
            peak_raw = float(Vm_finite[int(spikes_in_window[0])]) if int(spikes_in_window[0]) < len(Vm_finite) else np.nan
            baseline = _local_baseline(int(spikes_in_window[0]))
            peak_amp = peak_raw - baseline if baseline_subtract else peak_raw
            burst_metrics.append({
                'start': int(spikes_in_window[0]),
                'end': int(spikes_in_window[0]),
                'n_spikes': 1,
                'peak_amp': peak_amp,
                'peak_z': peak_z,
                'baseline': baseline,
    ...
    for spk in unassigned_spikes:
        ...
        peak_z = float(Vm_z_finite[spk]) if spk < len(Vm_z_finite) else np.nan
        burst_metrics.append({
            'start': spk,
            'end': spk,
            'n_spikes': 1,
    ...
    # Step 1: merge complex bursts that are close in time.
    ...
    # Step 2: absorb nearby simple spikes into complex bursts.
...
def complex_bursts_detection_segment(
    ...
    Pipeline (same spirit as your original):
      ...
      5) IMPORTANT: If a bracket contains multiple WMF peaks, split it at the deepest trough(s)
         between peaks so bursts donג€™t merge.
    ...
    # Remove around isolated single spikes (ֲ±1 sample)
    for t in single_spikes:
        if 0 <= t < n:
    ...
    else:  # 'volpy_based' ג€” bracket by troughs
...
def spike_height_calculation(refined_SS, trace_idx, trace_idx_mf, trace_noCS, fr, moving_window_size=20, overlap=10, plotflag=False):
    ...
    if plotflag:
        ...
        if exp_fit_success_simple:
            axes[0].plot(moving_avg_times_simple_sec, exp_fit_simple, '--', color='red', linewidth=2, label=f'Exp decay (ֿ„={1/decay_coeff_simple:.2f}s)')
        ...
        if exp_fit_success_noise:
            axes[1].plot(moving_avg_times_simple_sec, exp_fit_noise, '--', color='red', linewidth=2, label=f'Exp decay (ֿ„={1/decay_coeff_noise:.2f}s)')
        ...
        if exp_fit_success_snr:
            axes[2].plot(moving_avg_times_simple_sec, exp_fit_snr, '--', color='red', linewidth=2, label=f'Exp decay (ֿ„={1/decay_coeff_snr:.2f}s)')
    ...
    if exp_fit_success_simple:
        ...
        print(f"Simple Spikes - Time constant (ֿ„ = 1/b): {1/decay_coeff_simple:.2f} seconds")
    ...
    if exp_fit_success_noise:
        ...
        print(f"Baseline Noise - Time constant (ֿ„ = 1/b): {1/decay_coeff_noise:.2f} seconds")
    print(f"SNR - Linear slope: {linear_slope_snr:.6f}")
    if exp_fit_success_snr:
        print(f"SNR - Exponential decay coefficient (b): {decay_coeff_snr:.6f}")
        print(f"SNR - Time constant (ֿ„ = 1/b): {1/decay_coeff_snr:.2f} seconds")
...
def plot_burst_metrics_pdf(burst_metrics, pdf=None, save_path=None, figsize=(6, 3),
                           simple_spike_color='#026C80', complex_spike_color='#EE9B00'):
    """
    Plot burst amplitude/duration and AUC vs spike count with mean ֲ± s.e.m. lines.
    """
    ...
    if len(unique_counts) > 0:
        axes[1].errorbar(unique_counts, means_simple, yerr=sems_simple, color=simple_spike_color, linestyle='-', marker='o', markersize=4, capsize=3, label='Simple mean ֲ± s.e.m.')
        axes[1].errorbar(unique_counts, means_complex, yerr=sems_complex, color=complex_spike_color, linestyle='-', marker='o', markersize=4, capsize=3, label='Complex mean ֲ± s.e.m.')
        axes[1].errorbar(unique_counts, means_all, yerr=sems_all, color='black', linestyle='-', marker='o', markersize=4, capsize=3, label='All mean ֲ± s.e.m.')
